use crate::{
    proc_block_v1::{ GraphError, KernelError, InvalidInput, BadInputReason},
    runtime_v1::*,
};

use hotg_rune_proc_blocks::{
    ndarray::ArrayViewMut1, BufferExt, ValueType,
};
...
fn softmax<T>(mut input:  ArrayViewMut1<T>) where T: Float + num_traits::FromPrimitive {
        input.mapv_inplace(|x| x.exp());
        match input.mean() {
            Some(sum) => input.mapv_inplace(|x| x / sum),
            _  => {}
        }
 }
...
impl proc_block_v1::ProcBlockV1 for ProcBlockV1 {
    fn register_metadata(){
        ...
        let hint =
            supported_shapes(&[ElementType::F32, ElementType::F64], Dimensions::Fixed(&[0]));
        input.add_hint(&hint);
        metadata.add_input(&input);

        let soft_max = TensorMetadata::new("soft_max");
        soft_max.set_description("Vector normalised into probability distribution");
        let hint =
            supported_shapes(&[ElementType::F32, ElementType::F64], Dimensions::Fixed(&[0]));
    ...
    fn kernel(id: String)-> Result<(), KernelError> {
        ...
        match element_type {
            ElementType::F32 => preprocess_buffer::<f32>(&mut buffer, &dimensions).map(|t| softmax(t))?,
            ElementType::F64 => preprocess_buffer::<f64>(&mut buffer, &dimensions).map(|t| softmax(t))?,
            other => {
